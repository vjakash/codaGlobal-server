...
router.put('/', [authenticate], async(req, res) => {
    let { postId, likedPerson } = req.body;
    if (postId === undefined || likedPerson === undefined) {
        res.status(400).json({
        ...
        await db.collection('posts').updateOne({ _id }, { $push: { likes: likedPerson } }).catch((err) => { throw err; });
